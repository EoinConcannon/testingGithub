...
import { useParams } from 'react-router-dom'; //allows you to pull id out of URL
...
export default function Edit(props) {
    ...
    }, []);//empty array stops useEffect from looping multiple times
    const handleSubmit = (event) => {
        ...
        axios.put('http://localhost:4000/api/book/' + id, newBook)
            .then((res) => {
                ...
                navigate('/read');//goes back to read page when value is got
            })
            .catch((error) => {
                console.log(error);//error page for project?
            });
    }
    return (
        <div>
